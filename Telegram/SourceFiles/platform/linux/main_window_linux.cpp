...
QImage _trayIconImageGen() {
	...
	if (_trayIconImage.isNull() || _trayIconImage.width() != _trayIconSize || muted != _trayIconMuted || counterSlice != _trayIconCount) {
		if (_trayIconImageBack.isNull() || _trayIconImageBack.width() != _trayIconSize) {
			_trayIconImageBack = Core::App().logoForkgramNoMargin().scaled(_trayIconSize, _trayIconSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
			...
			for (int32 y = 0; y < h; ++y) {
				for (int32 x = 0; x < w; ++x) {
					...
					bytes[srcoff + QT_RED  ] = qMax(bytes[srcoff + QT_RED  ], uchar(58));
					bytes[srcoff + QT_GREEN] = qMax(bytes[srcoff + QT_GREEN], uchar(46));
					bytes[srcoff + QT_BLUE ] = qMax(bytes[srcoff + QT_BLUE ], uchar(92));
				}
			}
		}
